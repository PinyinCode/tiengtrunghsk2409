...
body.practice-full-open .onboarding-active-banner{display:none!important}
...
.onboarding-counter.full {
    background: linear-gradient(135deg, rgba(245,158,11,.15), rgba(251,191,36,.-color: rgba(245,158,11,.5);
    color: #92400e;
}
...
/* ══════════════════════════════════════════════════════════════ */
/* BANNER GỘP: Chủ đề + Trial + Lock stats + CTA                  */
/* ══════════════════════════════════════════════════════════════ */
...
.onboarding-active-banner .ob-count {
   anner color: var(--text-3);
    font .-size: .75rem;
   ob font-weight: 600;
    white-change-space: nowrap;
    flex-shr-btnink: 0;
}
...
/* Dòng 2: Trial info + Lock stats + CTA */
...
/* ═══════════════════════════════════════════════════════════ */
/* Style theo tier: Demo / Trial / Urgent / Expired            */
/* ═══════════════════════════════════════════════════════════ */
.onboarding-active-banner.ob-tier-demo {
    background: linear-gradient(135deg,
        rgba(59,130,246,.10),
        rgba(99,102,241,.08));
    border-color: rgba(59,130,246,.4);
}
.onboarding-active-banner.ob-tier-demo .ob-icon {
    background: linear-gradient(135deg, rgba(59,130,246,.18), rgba(99,102,241,.15));
    color: #2563eb;
}

.onboarding-active-banner.ob-tier-urgent {
    background: linear-gradient(135deg,
        rgba(245,158,11,.12),
        rgba(234,88,12,.10));
    border-color: rgba(234,88,12,.45);
}
.onboarding-active-banner.ob-tier-urgent .ob-icon {
    background: linear-gradient(135deg, rgba(234,88,12,.22), rgba(194,65,12,.18));
    color: #ea580c;
}
.onboarding-active-banner.ob-tier-urgent .ob-stat-tier {
    border-color: rgba(234,88,12,.5);
    background: rgba(255,237,213,.7);
}

.onboarding-active-banner.ob-tier-expired {
    background: linear-gradient(135deg,
        rgba(220,38,38,.12),
        rgba(185,28,28,.10));
    border-color: rgba(220,38,38,.5);
}
.onboarding-active-banner.ob-tier-expired .ob-icon {
    background: linear-gradient(135deg, rgba(220,38,38,.22), rgba(153,27,27,.18));
    color: #dc2626;
}
.onboarding-active-banner.ob-tier-expired .ob-stat-tier {
    border-color: rgba(220,38,38,.5);
    background: rgba(254,226,226,.75);
}

[data-theme="dark"] .onboarding-active-banner.ob-tier-urgent .ob-stat-tier {
    background: rgba(124,45,18,.5);
}
[data-theme="dark"] .onboarding-active-banner.ob-tier-expired .ob-stat-tier {
    background: rgba(127,29,29,.5);
}

.onboarding-active-banner .ob-stat-tier {
    background: rgba(255,255,255,.85);
    border-color: rgba(245,158,11,.5);
}
.onboarding-active-banner .ob-stat-tier .ob-stat-icon {
    background: linear-gradient(135deg, #fef3c7, #fde68a);
    color: #b45309;
}
[data-theme="dark"] .onboarding-active-banner .ob-stat-tier {
    background: rgba(30,41,59,.7);
    border-color: rgba(245,158,11,.55);
}
[data-theme="dark"] .onboarding-active-banner .ob-stat-tier .ob-stat-icon {
    background: linear-gradient(135deg, rgba(245,158,11,.3), rgba(217,119,6,.22));
    color: #fcd34d;
}

.onboarding-active-banner .ob-stat-sub {
    color: var(--text-3);
    font-size: .9em;
    font-weight: 50024;
    margin-left: .15em;
...
[data-theme="dark"] .onboarding-active-banner.ob-tier-demo {
    background: linear-gradient(135deg,
        rgba(59,130,246,.18),
        rgba(99,102,241,.15));
    border-color: rgba(96,165,250,.45);
}
[data-theme="dark"] .onboarding-active-banner.ob-tier-trial {
    background: linear-gradient(135deg,
        rgba(99,102,241,.15),
        rgba(139,92,246,.15));
}
[data-theme="dark"] .onboarding-active-banner.ob-tier-urgent {
    background: linear-gradient(135deg,
        rgba(234,88,12,.20),
        rgba(194,65,12,.18));
    border-color: rgba(251,146,60,.5);
}
[data-theme="dark"] .onboarding-active-banner.ob-tier-expired {
    background: linear-gradient(135deg,
        rgba(220,38,38,.22),
        rgba(185,28,28,.18));
...
function initDatasetSelector() {
    ...
    /* Build subGrid bằng DOM API để giữ nguyên dấu tiếng Việt */
...
function updateDemoRemaining() {
    /* Không còn demoRemainingText trong DOM (đã gộp banner) — no-op */
}
...
function getOnboardingConfig() {
    ...
    if (info.tier === 'demo')  return ONBOARDING_CONFIG.demo  || null;
    if (info.tier === 'trial') return ONBOARDING_CONFIG.trial || null;
    return null;
}
...
function maybeShowOnboarding() {
    var info = getTierInfo();
    if (info.tier !== 'demo' && info.tier !== 'trial') return;
...
function showOnboardingModal() {
    ...
    /* Load lại selection đã lưu nếu có */
    var saved = loadOnboardingSelection();
    ...
    var maxCountEl = $('onboardingMaxCount');
    if (titleEl) titleEl.textContent = cfg.title || 'Bạn quan tâm chủ đề nào?';
    if (subtitleEl) subtitleEl.textContent = cfg.subtitle || '';
    if (maxCountEl) maxCountEl.textContent = cfg.topics_per_user || 3;
...
function onToggleOnboardingTopic(topicName) {
    ...
    var max = _onboardingConfig.topics_per_user || 3;
...
function updateOnboardingUI() {
    ...
    if (counter) {
        ...
        if (selected >= max) counter.classList.add('full');
    }
...
/* ═══════════════════════════════════════════════════════════ */
/* XỬ LÝ NÚT "ĐỔI CHỦ ĐỀ" — FIX BUG TRIAL                       */
/* ═══════════════════════════════════════════════════════════ */
function onChangeTopicsClick() {
    ...
    if (!cfg) {
        ...
        if (typeof ONBOARDING_CONFIG !== 'undefined' && ONBOARDING_CONFIG) {
            if (info.tier === 'demo')  cfg = ONBOARDING_CONFIG.demo  || null;
            else if (info.tier === 'trial') cfg = ONBOARDING_CONFIG.trial || null;
        }
        if (!cfg) {
...
/* ═══════════════════════════════════════════════════════════ */
/* BANNER GỘP: Chủ đề + Trial/Expiry + Lock stats + CTA         */
/* ═══════════════════════════════════════════════════════════ */
function showOnboardingActiveBanner(topics, count) {
    ...
    var isDemo = (info.tier === 'demo');
    var isTrial = (info.tier === 'trial');
    var isExpired = (info.tier === 'expired');

    /* Đếm số chuyên ngành bị khoá */
    ...
    /* Lấy số ngày còn lại (nếu có) */
    var daysLeft = null;
    var expDateStr = '';
    if (!isDemo && !isExpired && typeof currentUser !== 'undefined' && currentUser) {
        if (typeof getDaysRemaining === 'function') {
            daysLeft = getDaysRemaining(currentUser);
        }
        if (currentUser.expiresAt && typeof getExpiryDate === 'function') {
            var expDate = getExpiryDate(currentUser.expiresAt);
            if (expDate && !isNaN(expDate.getTime())) {
                expDateStr = expDate.toLocaleDateString('vi-VN');
            }
        }
    }

    /* ── Xác định style banner theo tier ── */
    var banner = document.createElement('div');
    banner.id = 'onboardingActiveBanner';
    banner.className = 'onboarding-active-banner';

    if (isDemo) {
        banner.classList.add('ob-tier-demo');
    } else if (isExpired) {
        banner.classList.add('ob-tier-expired');
    } else if (isTrial && daysLeft !== null && daysLeft <= 3) {
        banner.classList.add('ob-tier-urgent');
    } else if (isTrial) {
        banner.classList.add('ob-tier-trial');
    }

    /* ═══ HÀNG 1: Chủ đề + chips + count + nút Đổi ═══ */
    ...
    /* ═══ HÀNG 2: Trial info + Lock stats + CTA ═══ */
    var showRow2 = isLimited && (lockedCount > 0 || lockedIndustryCount > 0 ||
                                 isDemo || isExpired || (isTrial && daysLeft !== null));

    if (showRow2) {
        var statsBar = document.createElement('div');
        statsBar.className = 'ob-stats-bar';

        /* ── Chip 1: Trial còn X ngày / Demo / Hết hạn ── */
        var tierChip = document.createElement('div');
        tierChip.className = 'ob-stat-item ob-stat-tier';

        if (isDemo) {
            tierChip.innerHTML =
                '<span class="ob-stat-icon"><i class="fas fa-gift"></i></span>' +
                '<span class="ob-stat-text">Đang xem bản <b>Demo</b> giới hạn</span>';
        } else if (isExpired) {
            tierChip.innerHTML =
                '<span class="ob-stat-icon"><i class="fas fa-exclamation-triangle"></i></span>' +
                '<span class="ob-stat-text">Tài khoản đã <b>HẾT HẠN</b></span>';
        } else if (isTrial && daysLeft !== null) {
            var expLine = expDateStr ? ' <span class="ob-stat-sub">(đến ' + expDateStr + ')</span>' : '';
            if (daysLeft <= 0) {
                tierChip.innerHTML =
                    '<span class="ob-stat-icon"><i class="fas fa-exclamation-circle"></i></span>' +
                    '<span class="ob-stat-text">Trial đã <b>hết hạn</b></span>';
            } else {
                tierChip.innerHTML =
                    '<span class="ob-stat-icon"><i class="fas fa-hourglass-half"></i></span>' +
                    '<span class="ob-stat-text">Trial còn <b>' + daysLeft + ' ngày</b>' + expLine + '</span>';
            }
        }
        if (tierChip.innerHTML) statsBar.appendChild(tierChip);

        /* ── Chip 2: Số câu chưa mở ── */
        if (lockedCount > 0) {
        ...
        /* ── Chip 3: Số chuyên ngành chưa mở ── */
        ...
        /* ── Nút CTA duy nhất ── */
        if (lockedCount > 0 || lockedIndustryCount > 0 || isDemo || isExpired) {
            var ctaBtn = document.createElement('button');
            ctaBtn.type = 'button';
            ctaBtn.className = 'ob-cta-btn';

            if (isDemo) {
                ctaBtn.innerHTML = '<i class="fas fa-sign-in-alt"></i> Đăng nhập để mở khoá';
                ...
                ctaBtn.innerHTML = '<i class="fas fa-gem"></i> Nâng cấp ngay';
                ctaBtn.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    if (typeof openRenewalModal === 'function') openRenewalModal();
...
function refreshApp() {
    ...
    if (typeof maybeShowOnboarding === 'function') {
        setTimeout(function() {
            maybeShowOnboarding();
            /* Refresh banner để update số ngày trial/demo */
            var oldBanner = $('onboardingActiveBanner');
            if (oldBanner && window.__onboardingOverride && window.__onboardingOverride.length > 0) {
                var saved = loadOnboardingSelection();
                if (saved && saved.topics) {
                    showOnboardingActiveBanner(saved.topics, window.__onboardingOverride.length);
                }
            }
        }, 800);
    }
...
window.speakText = function(text, btn, evt) {
    ...
    if (shouldCountUsage()) { incDemoUsage(); }
...
function populateVoiceSelect() {
    ...
    if (!zhVoices.length) {
        sel.innerHTML = '<option value="">-- Đang tải giọ   ng đọc... --</option>';
        return;
    sel }
.innerHTML = '';
...
/* DATASET SELECTOR trong Practice Full — dùng DOM API để giữ ký tự đặc biệt */
