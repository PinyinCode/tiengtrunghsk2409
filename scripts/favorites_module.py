...
async function favAdd(stt, record) {
    ...
    /* ═══════════════════════════════════════════════════════════
       LỚP 1: GIỚI HẠN TỔNG SỐ TIM (500)
       ═══════════════════════════════════════════════════════════ */
    var MAX_TOTAL = 500;
    if (!isAdmin && favCount() >= MAX_TOTAL) {
        favShowToast('Đã đạt giới hạn ' + MAX_TOTAL + ' câu yêu thích', 'warn');
        return false;
    }

    /* ═══════════════════════════════════════════════════════════
       LỚP 2: RATE LIMIT (20 TIM/PHÚT)
       ═══════════════════════════════════════════════════════════ */
    var MAX_PER_MIN = 20;
    var WINDOW_MS = 60000;

    if (!isAdmin) {
        var rateKey = 'fav_rate_' + email;
        var timestamps = [];
        try {
            timestamps = JSON.parse(localStorage.getItem(rateKey) || '[]');
        } catch(e) { timestamps = []; }

        var now = Date.now();
        /* Lọc timestamp còn trong 1 phút */
        timestamps = timestamps.filter(function(t) {
            return (now - t) < WINDOW_MS;
        });

        if (timestamps.length >= MAX_PER_MIN) {
            var waitSec = Math.ceil((WINDOW_MS - (now - timestamps[0])) / 1000);
            favShowToast('Thao tác quá nhanh! Chờ ' + waitSec + 's', 'warn');
            return false;
        }
        ...
        timestamps.push(now);
        try {
            localStorage.setItem(rateKey, JSON.stringify(timestamps));
        } catch(e) {}
    }

    /* ═══════════════════════════════════════════════════════════
       LỚP 3: LƯU LOCAL — OPTIMISTIC UPDATE (UI phản hồi ngay)
       ═══════════════════════════════════════════════════════════ */
    favState.items[stt] = {
        stt: stt,
        addedAt: Date.now(),
