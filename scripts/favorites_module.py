...
Đặc điểm:
  - Nút ❤️ FLOAT góc dưới phải PF: NHẠT khi chưa thích, ĐẬM khi đã thích
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Trạng thái: NHẠT (mặc định) — ĐẬM (đã thích) — KHOÁ (chưa tier)
   ═══════════════════════════════════════════════════════════════ */

/* Trạng thái MẶC ĐỊNH — câu CHƯA thích → nhạt */
.pf-fav-float{
    position:fixed;
...
/* Trạng thái ĐÃ THÍCH — câu đang yêu thích → đậm */
.pf-fav-float.active{
...
.pf-fav-float.locked i{color:#dc2626;}
...
[data-theme="dark"] .pf-fav-float i{color:#94a3b8;}
[data-theme="dark"] .pf-fav-float:hover{
    background:linear-gradient(135deg,rgba(239,68,68,.15),rgba(220,38,38,.1));
    color:#fca5a5;
    border-color:#ef4444;
}
[data-theme="dark"] .pf-fav-float:hover i{color:#fca5a5;}
[data-theme="dark"] .pf-fav-float.active{
    background:linear-gradient(135deg,#ef4444,#dc2626);
    color:#fff;
}
[data-theme="dark"] .pf-fav-float.active i{color:#fff;}
[data-theme="dark"] .pf-fav-float.locked{
    color:#fca5a5;
}
[data-theme="dark"] .pf-fav-float.locked i{color:#fca5a5;}
...
function favNotifyChanged() {
    ...
    if (typeof favUpdatePFDropdownState === 'function') favUpdatePFDropdownState();
...
function favBuildCardHtml(r) {
    ...
    var favBtn = '<button class="fav-btn' +
        (active ? ' active' : ' fa') +
        (!can ? ' locked' : '') +
-he        '" data-stt="' + sttartSafe + '" ' +
        'onclick="'))favOnCardBtnClick(event, \'' + st +tJs + '\')" ' +
        ' 'title="' + (active ? 'Xoá khỏi yêu thích' : 'Thêm vào yêu thích') + '">' +
        '<i class="' + (!can ? 'fas fa-lock' : (active ? 'fas fa-heart' : 'far"></i>' +
    '</button>';
...
/* ═══════════════════════════════════════════════════════════════
   ❤️ INJECT OPTION "YÊU THÍCH" VÀO DROPDOWN PF
   ═══════════════════════════════════════════════════════════════ */
...
function initFavorites() {
    ...
    /* Watch PF modal mở → inject option + sync nút float */
            ...
            var observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(m) {
                    if (m.attributeName === 'class') {
                        ...
                        if (isOpen) {
                            /* Đợi pfBuildDatasetSelect() chạy xong */
                            setTimeout(function() {
