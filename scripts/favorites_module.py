...
/* ═══════════════════════════════════════════════════════════════
   ★ PATCH: ĐỔI BỘ DỮ LIỆU — RESET TOGGLE + CHECK CÂU MỚI
   
   Khi user đổi #pfDatasetSelect (dropdown "Bộ dữ liệu"):
   1. LUÔN reset toggle "Chỉ câu yêu thích" về TẮT
   2. Đợi câu mới load xong → check câu mới có like không
   3. Cập nhật nút toggle (sáng/mờ) + nút tim (đỏ/xám)
   ═══════════════════════════════════════════════════════════════ */
(function() {
    'use strict';
    ...
    function handleDatasetChange() {
        console.log('[Favorites] Đổi bộ dữ liệu → bắt đầu đồng bộ');

        /* ═══ 1. RESET toggle về TẮT ═══ */
        ...
        /* ═══ 2. Đợi câu mới load → check + update UI ═══ */
        setTimeout(function() {
            /* A. Cập nhật nút tim FLOAT theo câu mới */
            if (typeof favUpdatePfFloatBtn === 'function') {
                favUpdatePfFloatBtn();
            }

            /* B. Cập nhật nút toggle theo câu mới */
            if (typeof favUpdatePfOnlyFavBtn === 'function') {
                favUpdatePfOnlyFavBtn();
            }

            /* C. Refresh dropdown "CÂU:" */
            if (typeof favRefreshQuestionDropdown === 'function') {
                favRefreshQuestionDropdown();
            }
            ...
            var curStt = (typeof pfCurrentStt !== 'undefined' && pfCurrentStt)
                         ? String(pfCurrentStt)
                         : null;
            var isFav = curStt && typeof favHas === 'function' ? favHas(curStt) : false;
            console.log('[Favorites] Câu mới #' + curStt + ' | liked: ' + isFav);

        }, 250); /* Delay 250ms để ui_template load câu xong */

        /* ═══ 3. Backup: chạy lại sau 500ms phòng khi chậm ═══ */
        setTimeout(function() {
            if (typeof favUpdatePfFloatBtn === 'function') {
                favUpdatePfFloatBtn();
            }
    ...
    function attachListener() {
        ...
        sel.addEventListener('change', function() {
            console.log('[Favorites] #pfDatasetSelect change event fired');
            /* Delay để ui_template switch RAW_DATA + loadPracticeFull */
            setTimeout(handleDatasetChange, 200);
        });
