...
function favLoadFromLocal() {
    try {
        ...
        if (cached && cached.items && typeof cached.items === 'object') {
            ...
            /* ⭐ MIGRATION: gán datasetId mặc định 'tonghop' cho item cũ */
            ...
            Object.keys(favState.items).forEach(function(k) {
                ...
                if (item && !item.datasetId) {
                    item.datasetId = 'tonghop';  /* Mặc định dataset gốc */
                    needResave = true;
                }
            });
            if (needResave) favSaveToLocal();
    ...
    try {
        ...
        snap.forEach(function(doc) {
            ...
            items[sttKey] = {
                ...
                hsk: d.hsk || '',
                subject: d.subject || '',
                datasetId: d.datasetId || 'tonghop'   /* ⭐ Fallback nếu cloud cũ */
            };
        });
    ...
    favState.listener = db.collection('favorites').doc(email)
        ...
        .onSnapshot(function(snap) {
            ...
            snap.forEach(function(doc) {
                var d = doc.data() || {};
                var sttKey = String(doc.id);
                var dsId = d.datasetId || null;

                if (!dsId) {
                    /* Item cloud cũ thiếu datasetId → đánh dấu cần migrate */
                    dsId = 'tonghop';
                    needMigrate = true;
                }
            ...
            /* ⭐ Nếu có item cũ thiếu datasetId → migrate lên cloud */
            if (needMigrate) {
...
/* ═══════════════════════════════════════════════════════════════
   ⭐ MIGRATION: Đẩy datasetId mặc định lên Firestore cho item cũ
   Chạy nền, không block UI, có guard chống chạy trùng
   ═══════════════════════════════════════════════════════════════ */
...
async function favMigrateLocalToCloud() {
    ...
    /* Thu thập các item cần migrate (thiếu datasetId trên cloud) */
    var candidates = [];
    Object.keys(favState.items).forEach(function(k) {
        var item = favState.items[k];
        if (!item) return;
        /* Chỉ migrate những item còn trong state (không bị xoá) */
        candidates.push({ stt: String(k), datasetId: item.datasetId || 'tonghop' });
    });
    ...
    try {
        ...
        snap.forEach(function(doc) {
            var d = doc.data() || {};
            if (!d.datasetId) {
                /* Cloud thiếu → cần update */
                var localItem = favState.items[String(doc.id)];
                toFix.push({
                    ref: doc.ref,
                    datasetId: (localItem && localItem.datasetId) ? localItem.datasetId : 'tonghop'
                });
            }
        });
        ...
        for (var i = 0; i < toFix.length; i += BATCH_SIZE) {
            var batch = db.batch();
            var chunk = toFix.slice(i, i + BATCH_SIZE);
            chunk.forEach(function(entry) {
                batch.update(entry.ref, { datasetId: entry.datasetId });
            });
            await batch.commit();
        }

        console.log('[Favorites] Migrated ' + toFix.length + ' items to cloud');

    } catch(e) {
        console.warn('[Favorites] Migration cloud error:', e);
    } finally {
        _favMigrating = false;
    }
}
...
async function favAdd(stt, record) {
    ...
    /* ═══════════════════════════════════════════════════════════
       ⭐ LẤY datasetId HIỆN TẠI — ĐỂ LOOKUP ĐÚNG DATASET SAU NÀY
       ═══════════════════════════════════════════════════════════ */
    var currentDsId = (typeof CURRENT_DATASET !== 'undefined' && CURRENT_DATASET)
                      ? CURRENT_DATASET
                      : 'tonghop';
    ...
    favState.items[stt] = {
        ...
        datasetId: currentDsId   /* ⭐ QUAN TRỌNG */
    };
    favSaveToLocal();
    favNotifyChanged();
...
/* ═══════════════════════════════════════════════════════════════
   ⭐ TÌM RECORD THEO stt TRONG 1 DATASET CỤ THỂ
   Dùng datasetId đã lưu để lookup chính xác — tránh nhầm câu
   ═══════════════════════════════════════════════════════════════ */
function favFindRecordInDataset(dsId, stt) {
    ...
    /* ─── 2. Fallback cho dataset 'tonghop' qua RAW_DATA ─── */
...
/* ═══════════════════════════════════════════════════════════════
   ⭐ TÌM RECORD Ở BẤT KỲ DATASET NÀO (fallback cuối cùng)
   Chỉ dùng khi không xác định được datasetId
   ═══════════════════════════════════════════════════════════════ */
function favFindRecordAnywhere(stt) {
    if (stt === null || stt === undefined) return null;
    stt = String(stt);

    /* ─── 1. Quét tất cả dataset trong REGISTRY ─── */
    if (typeof DATASET_REGISTRY !== 'undefined' && DATASET_REGISTRY) {
...
/* ═══════════════════════════════════════════════════════════════
   ⭐ FIX: LẤY RECORDS CHÍNH XÁC THEO datasetId ĐÃ LƯU
   Mỗi favorite được lưu kèm datasetId → lookup đúng dataset đó
   ═══════════════════════════════════════════════════════════════ */
function favGetRecords() {
    var out = [];
    var seen = {};

    /* Duyệt tất cả favorite items */
    Object.keys(favState.items).forEach(function(sttKey) {
        var item = favState.items[sttKey];
        if (!item) return;

        var targetDs = item.datasetId || 'tonghop';

        /* Tìm record trong đúng dataset đó */
        var record = favFindRecordInDataset(targetDs, item.stt);
        if (!record) {
            /* Fallback: thử tìm trong dataset hiện tại */
            var currentDs = (typeof CURRENT_DATASET !== 'undefined') ? CURRENT_DATASET : 'tonghop';
            record = favFindRecordInDataset(currentDs, item.stt);
        }
        if (!record) {
            /* Fallback 2: quét tất cả dataset, lấy cái đầu tiên khớp */
            record = favFindRecordAnywhere(item.stt);
        }

        if (record && !seen[String(record.stt) + '::' + targetDs]) {
            seen[String(record.stt) + '::' + targetDs] = true;
            out.push(record);
        }
    });
...
function favFindRecordInDataset(dsId, stt) {
    ...
    /* Fallback cho dataset tonghop khi không có DATASET_REGISTRY */
    if (dsId === 'tonghop' && typeof RAW_DATA !== 'undefined' && RAW_DATA) {
        for (var j = 0; j < RAW_DATA.length; j++) {
            if (String(RAW_DATA[j].stt) === stt) return RAW_DATA[j];
        }
...
/* Quét tất cả dataset — chỉ dùng làm fallback cuối */
function favFindRecordAnywhere(stt) {
    if (stt === null || stt === undefined) return null;
    stt = String(stt);
...
window.favOnCardBtnClick = function(evt, stt) {
    ...
    /* ═══════════════════════════════════════════════════════════
       ⭐ TÌM RECORD TRONG DATASET HIỆN TẠI (không quét RAW_DATA)
       ═══════════════════════════════════════════════════════════ */
    var currentDsId = (typeof CURRENT_DATASET !== 'undefined' && CURRENT_DATASET)
                      ? CURRENT_DATASET
                      : 'tonghop';
    ...
    /* Cách 1: Tìm trong dataset hiện tại */
    if (typeof favFindRecordInDataset === 'function') {
        record = favFindRecordInDataset(currentDsId, stt);
    }

    /* Cách 2: Fallback — tìm ở bất kỳ dataset */
    if (!record && typeof favFindRecordAnywhere === 'function') {
        record = favFindRecordAnywhere(stt);
    }

    /* Cách 3: Fallback cuối — quét RAW_DATA */
    if (!record && typeof RAW_DATA !== 'undefined') {
        for (var i = 0; i < RAW_DATA.length; i++) {
            if (String(RAW_DATA[i].stt) === String(stt)) {
                record = RAW_DATA[i];
                break;
            }
        }
...
window.favOnPfBtnClick = function(evt) {
    ...
    /* ⭐ Tìm record trong dataset hiện tại */
    var currentDsId = (typeof CURRENT_DATASET !== 'undefined' && CURRENT_DATASET)
                      ? CURRENT_DATASET
                      : 'tonghop';
    ...
    if (!record && typeof RAW_DATA !== 'undefined') {
        for (var i = 0; i < RAW_DATA.length; i++) {
            if (String(RAW_DATA[i].stt) === stt) { record = RAW_DATA[i]; break; }
        }
    }

    favToggle(stt, record).then(function() {
        favUpdatePfFloatBtn();
    });
};
